#!/usr/bin/env python3
"""hack/bench/bench.py — Runtime Enforcer resource-consumption benchmark

Usage:
    ./hack/bench/bench.py [--namespace NS] [--testimage IMAGE_PATH] [--nodename NODE_NAME]

Output: Markdown report in hack/bench/results/YYYYMMDD-HHMMSS.md
"""

import argparse
import json
import shutil
import statistics
import string
import subprocess
import sys
import time
import textwrap
import yaml
import shlex
from tabulate import tabulate

from datetime import datetime, timezone
from pathlib import Path
from typing import Dict, List, Optional, Tuple

baseline_testcase = (0, 10, 50, 100)
baseline_header = [ "Pod count", "Node CPU avg (m)", "Node Memory avg (Mi)", "NoPolicy execve throughput(ops/sec)" ]

pod_testcase = (0, 10, 50, 100)
pod_header = [ "Pod count", "Node CPU avg (m)", "Node Memory avg (Mi)", "Agent CPU avg (m)", "Agent Memory avg (Mi)", "Controller CPU avg (m)", "Controller Memory avg (Mi)", "NoPolicy execve throughput(ops/sec)", "Monitor execve throughput(ops/sec)", "Protect execve throughput(ops/sec)"]

policy_testcase = (1, 10, 30, 50)
policy_header = [ "Pod count", "Node CPU avg (m)", "Node Memory avg (Mi)", "Agent CPU avg (m)", "Agent Memory avg (Mi)", "Controller CPU avg (m)", "Controller Memory avg (Mi)", "NoPolicy execve throughput(ops/sec)", "Monitor execve throughput(ops/sec)", "Protect execve throughput(ops/sec)"]

MANIFESTS_DIR = Path(__file__).parent / "manifests"
CHART_DIR = Path(__file__).parent.parent.parent / "charts" / "runtime-enforcer"

def load_manifest(filename: str, **kwargs: str) -> str:
    """Load a YAML manifest template from manifests/ and substitute variables."""
    template = string.Template((MANIFESTS_DIR / filename).read_text())
    return template.substitute(**kwargs)

# ---------------------------------------------------------------------------
# Globals (populated in main before any section runs)
# ---------------------------------------------------------------------------
AGENT_NS: str = ""
REPORT_PATH: Path = Path()
IMAGE_PATH = ""
NODE_NAME = ""


# ---------------------------------------------------------------------------
# CLI
# ---------------------------------------------------------------------------

def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Runtime Enforcer resource-consumption benchmark",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    p.add_argument(
        "--namespace", default="runtime-enforcer",
        help="Namespace where runtime-enforcer is deployed (default: runtime-enforcer)",
    )
    p.add_argument(
        "--testimage", default="",
        required=True,
        help="The test image path",
    )
    p.add_argument(
        "--nodename", default="",
        required=True,
        help="The node where tests will be running on",
    )
    return p.parse_args()


# ---------------------------------------------------------------------------
# Subprocess helpers
# ---------------------------------------------------------------------------

def run(
    cmd: List[str],
    *,
    input: Optional[str] = None,
    capture: bool = True,
) -> subprocess.CompletedProcess:
    """Run *cmd*, optionally feeding *input* to stdin. Never raises on non-zero exit."""
    return subprocess.run(
        cmd,
        input=input,
        capture_output=capture,
        text=True,
        check=False,
    )


def kubectl(*args: str, input: Optional[str] = None) -> subprocess.CompletedProcess:
    result = run(["kubectl"] + list(args), input=input)
    return result

def helm(*args: str) -> subprocess.CompletedProcess:
    result = run(["helm"] + list(args))
    if result.returncode != 0:
        print("Output:", result.stdout)
        print("Errors:", result.stderr)
        print("Return Code:", result.returncode)
    return result

# ---------------------------------------------------------------------------
# Logging / report helpers
# ---------------------------------------------------------------------------

def log(msg: str) -> None:
    print(f"[bench] {msg}", file=sys.stderr)


def die(msg: str) -> None:
    print(f"[bench] ERROR: {msg}", file=sys.stderr)
    sys.exit(1)


def report(line: str = "") -> None:
    """Append *line* to the report file and print it to stdout."""
    print(line)
    with REPORT_PATH.open("a") as f:
        f.write(line + "\n")

# ---------------------------------------------------------------------------
# Namespace helper
# ---------------------------------------------------------------------------
def recreate_namespace(ns: str, wait_timeout: int = 60) -> None:
    """
    Delete *ns* if it exists, wait for termination, then recreate it.
    This ensures no stale pods or policies from a prior run affect results.
    """
    kubectl("delete", "namespace", ns, "--ignore-not-found", "--wait=false")
    
    # Poll until the namespace is fully gone (or didn't exist)
    deadline = time.monotonic() + wait_timeout
    while time.monotonic() < deadline:
        r = kubectl("get", "namespace", ns, "--ignore-not-found", "--no-headers")
        if not r.stdout.strip():
            break
        time.sleep(3)
    else:
        die(f"namespace {ns} still terminating after {wait_timeout} s")
    kubectl("create", "namespace", ns)

def checkthroughput(mode: str, ns: str, labels: Optional[Dict[str, str]] = None) -> float:
    log(f"Checking throughput in {mode} mode...")
    if mode == "no-policy":
        kubectl("delete", "-f", "-", input=load_manifest(
            "benchmark-policy.yaml", namespace=ns, mode="monitor",
        ))
    else:
        kubectl("apply", "-f", "-", input=load_manifest(
            "benchmark-policy.yaml", namespace=ns, mode=mode,
        ))

    label_args = []
    if labels:
        label_str = ",".join(f"{k}={v}" for k, v in labels.items())
        label_args = [f"--labels={label_str}"]

    kubectl(
        "run", "benchmark-pod",
        "--image",
        IMAGE_PATH,
        "--restart=Never",
        f"--namespace={ns}",
        *label_args,
        "--overrides", json.dumps({"spec": {"nodeName": NODE_NAME}}),
        "--command", "--", "/usr/bin/sleep", "infinity",
    )
    kubectl(
        "wait", "pod", "benchmark-pod", f"-n={ns}",
        "--for=condition=Ready", "--timeout=60s",
    )

    ops_per_sec = measure_syscall(
        "benchmark-pod", "benchmark-pod", ns
    )

    log(f"execve in {mode} mode: {ops_per_sec} ops/second")


    kubectl(
        "delete", "pod", "benchmark-pod",
        f"-n={ns}", "--ignore-not-found",
    )

    return ops_per_sec


def sample_node_metrics(
    duration: int = 360, count: int = 3, interval: int = 30
) -> Tuple[int, int, int, int]:
    log(f"Collecting node metrics for {count} times in {interval}s interval")
    cpu_samples: List[int] = []
    mem_samples: List[int] = []
    end = time.monotonic() + duration

    metrics_collected = 0
    while time.monotonic() < end and metrics_collected < count:
        r = kubectl("top", "node", "--no-headers")
        if r.returncode == 0 and r.stdout.strip():
            parts = r.stdout.split()
            if len(parts) >= 4:
                cpu_samples.append(int(parts[1].rstrip("m")))
                mem_samples.append(int(parts[3].rstrip("Mi")))
                metrics_collected = metrics_collected + 1
        else:
            log("Failed to get metrics.  Retrying...")
        time.sleep(interval)

    if metrics_collected != count:
        die("WARNING: metrics timed out")

    return (
        int(statistics.mean(cpu_samples)),
        int(statistics.mean(mem_samples)),
        max(cpu_samples),
        max(mem_samples),
    )

def sample_agent_metrics(
    duration: int = 360, count: int = 3, interval: int = 30
) -> Tuple[int, int, int, int]:
    log(f"Collecting agent metrics for {count} times in {interval}s interval")
    cpu_samples: List[int] = []
    mem_samples: List[int] = []
    end = time.monotonic() + duration

    r = kubectl(
        "get", "pod", "-n", AGENT_NS,
        "-l", "app.kubernetes.io/component=agent",
        "--field-selector", f"spec.nodeName={NODE_NAME}",
        "-o", "jsonpath={.items[0].metadata.name}",
    )
    if r.returncode != 0:
        die("agent is not found")
    agent_pod = r.stdout.strip()
    if not agent_pod:
        die(f"agent pod not found in {AGENT_NS}")

    metrics_collected = 0
    while time.monotonic() < end and metrics_collected < count:
        r = kubectl("top", "pod", agent_pod, "-n", AGENT_NS, "--no-headers")
        if r.returncode == 0 and r.stdout.strip():
            parts = r.stdout.split()
            if len(parts) >= 3:
                cpu_samples.append(int(parts[1].rstrip("m")))
                mem_samples.append(int(parts[2].rstrip("Mi")))
                metrics_collected = metrics_collected + 1
        else:
            log("Failed to get metrics.  Retrying...")
        time.sleep(interval)

    if metrics_collected != count:
        log("WARNING: metrics timed out — reporting zeros")
        return 0, 0, 0, 0

    return (
        int(statistics.mean(cpu_samples)),
        int(statistics.mean(mem_samples)),
        max(cpu_samples),
        max(mem_samples),
    )

def sample_controller_metrics(
    duration: int = 360, count: int = 3, interval: int = 30
) -> Tuple[int, int, int, int]:
    log(f"Collecting controller metrics for {count} times in {interval}s interval")
    cpu_samples: List[int] = []
    mem_samples: List[int] = []
    end = time.monotonic() + duration

    r = kubectl(
        "get", "pod", "-n", AGENT_NS,
        "-l", "app.kubernetes.io/component=controller",
        "-o", "jsonpath={.items[0].metadata.name}",
    )
    if r.returncode != 0:
        die("controller is not found")
    controller_pod = r.stdout.strip()
    if not controller_pod:
        die(f"controller pod not found in {AGENT_NS}")

    metrics_collected = 0
    while time.monotonic() < end and metrics_collected < count:
        r = kubectl("top", "pod", controller_pod, "-n", AGENT_NS, "--no-headers")
        if r.returncode == 0 and r.stdout.strip():
            parts = r.stdout.split()
            if len(parts) >= 3:
                cpu_samples.append(int(parts[1].rstrip("m")))
                mem_samples.append(int(parts[2].rstrip("Mi")))
                metrics_collected = metrics_collected + 1
        else:
            log("Failed to get metrics.  Retrying...")
        time.sleep(interval)

    if metrics_collected != count:
        log("WARNING: metrics timed out — reporting zeros")
        return 0, 0, 0, 0

    return (
        int(statistics.mean(cpu_samples)),
        int(statistics.mean(mem_samples)),
        max(cpu_samples),
        max(mem_samples),
    )

def deploy_pause_pods(ns: str, count: int) -> None:
    log(f"Deploying {count} pause pods in {ns}...")

    r = kubectl("apply", "-f", "-", input=load_manifest(
        "pause-deployment.yaml",
        index=0,
        namespace=ns,
        replicas=count,
        node=NODE_NAME,
    ))
    if r.returncode != 0:
        die("failed to deploy pause deployment")

    r = kubectl(
        "rollout", "status", "deployment/pause-deployment", f"-n={ns}", "--timeout=180s",
    )
    if r.returncode != 0:
        die("failed to wait until deployment is rolled out")
    log("Wait for 30 seconds to make sure everything settles down")
    time.sleep(30)

def measure_syscall(
    pod: str, container: str, ns: str, repeat: int = 10000
) -> float:
    r = kubectl(
        "exec", pod, "-c", container, "-n", ns, "--",
        "/usr/bin/bash", "-c",
        "/usr/local/bin/perf bench syscall execve | tail -1 | awk '{print $1}'",
    )
    if r.returncode != 0:
        die("failed to run perf bench")
    raw = r.stdout.strip()

    try:
        return float(raw)
    except ValueError:
        die(f"unexpected perf output: {raw!r}")

# ---------------------------------------------------------------------------
# Main
# ---------------------------------------------------------------------------

def main() -> None:
    global AGENT_NS, REPORT_PATH, IMAGE_PATH, NODE_NAME

    args = parse_args()
    AGENT_NS = args.namespace

    script_dir = Path(__file__).parent
    results_dir = script_dir / "results"
    results_dir.mkdir(parents=True, exist_ok=True)

    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
    REPORT_PATH = results_dir / f"{timestamp}.md"
    REPORT_PATH.write_text("")  # create / truncate

    IMAGE_PATH = args.testimage
    NODE_NAME = args.nodename

    # ------------------------------------------------------------------
    # Preflight checks
    # ------------------------------------------------------------------
    for cmd in ("kubectl","helm"):
        if not shutil.which(cmd):
            die(f"{cmd} not found in PATH")

    r = kubectl("get", "namespace", "cert-manager")
    if r.returncode != 0:
        die("""cert-manager is not installed.  Add the helm repo and run below commands:

helm install cert-manager jetstack/cert-manager \\
--namespace cert-manager \\
--create-namespace \\
--set crds.enabled=true \\
--wait

helm install cert-manager-csi-driver jetstack/cert-manager-csi-driver \\
--namespace cert-manager \\
--wait
""")

    r = kubectl("get", "namespace", AGENT_NS)
    if r.returncode == 0:
        die(f"runtime-enforcer is already installed. In order to collect baseline metrics, please remove runtime-enforcer and its namespace.")

    r = kubectl("get", "node", NODE_NAME)
    if r.returncode != 0:
        die(f"the specified node name is not found")
    
    now = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    report("# Runtime Enforcer Resource Consumption Benchmark")
    report()
    report(f"**Generated:** {now}")
    report()

    # ------------------------------------------------------------------
    # Section 1: Baseline
    # ------------------------------------------------------------------

    report("## Baseline")
    
    baseline_ns = "bench-baseline"

    result = []
    for count in pod_testcase:
        log(f"Creating {count} test pods...")
        recreate_namespace(baseline_ns)
        deploy_pause_pods(baseline_ns, count)
        avg_nopolicy = checkthroughput("no-policy", baseline_ns)
        node_ac, node_am, node_mc, node_mm = sample_node_metrics()
        result.append([count, node_ac, node_am, avg_nopolicy])

        r = kubectl("delete", "namespace", baseline_ns, "--ignore-not-found")
        if r.returncode != 0:
            die("failed to delete namespace")

    report(tabulate(result, headers=baseline_header, tablefmt="github"))

    # ------------------------------------------------------------------
    # Section 2: Pod scaling
    # ------------------------------------------------------------------

    # Before everything, install runtime-enforcer
    log("Installing runtime enforcer...")
    helm("upgrade", "--install", "-n", "runtime-enforcer", "runtime-enforcer", CHART_DIR, "--wait", "--create-namespace", "--set", "controller.image.tag=latest", "--set", "agent.image.tag=latest")

    report("## Pod Scaling")
    report()
    report("Pause pods (registry.k8s.io/pause:3.10) are deployed in a dedicated namespace.")
    report()

    pod_scale_ns = "bench-pod-scaling"

    result = []
    for count in pod_testcase:
        log(f"Pod count: {count}")
        recreate_namespace(pod_scale_ns)

        r = kubectl("apply", "-f", "-", input=load_manifest(
            "pause-policy.yaml", namespace=pod_scale_ns,
        ))
        if r.returncode != 0:
            die("failed to apply policy")

        deploy_pause_pods(pod_scale_ns, count)
        if count != 0:
            time.sleep(60)  # wait for metrics-server to pick up new pod state

        ac, am, mc, mm = sample_agent_metrics()
        controller_ac, controller_am, controller_mc, controller_mm = sample_controller_metrics()

        node_ac, node_am, node_mc, node_mm = sample_node_metrics()

        avg_nopolicy = checkthroughput("no-policy", pod_scale_ns)
        avg_monitor = checkthroughput("monitor", pod_scale_ns, {"security.rancher.io/policy": "benchmark-policy"})
        avg_protect = checkthroughput("protect", pod_scale_ns, {"security.rancher.io/policy": "benchmark-policy"})

        result.append([count, node_ac, node_am, ac, am, controller_ac, controller_am, avg_nopolicy, avg_monitor, avg_protect])

        r = kubectl("delete", "namespace", pod_scale_ns, "--ignore-not-found")
        if r.returncode != 0:
            die("failed to delete namespace")

    report(tabulate(result, headers=pod_header, tablefmt="github"))


    # ------------------------------------------------------------------
    # Section 3: Policy scaling
    # ------------------------------------------------------------------
    report("## Policy Scaling")
    report()
    report("WorkloadPolicy CRs are created alongside one labeled pod per policy.")

    policy_ns = "bench-policy-scaling"
    log(f"  Cleaning up namespace {policy_ns} from any prior run...")
    recreate_namespace(policy_ns)

    result = []
    for n in policy_testcase:
        log(f"  Creating {n} policies and pods...")
        recreate_namespace(policy_ns)

        r = kubectl("apply", "-f", "-", input=load_manifest(
            "benchmark-policy.yaml", namespace=policy_ns, mode="monitor",
        ))
        if r.returncode != 0:
            die("failed to apply policy")

        # Create policies and pods.  This can take a while...
        for i in range(n):
            r = kubectl("apply", "-f", "-", input=load_manifest(
                "policy-scaling-policy.yaml",
                index=str(i),
                namespace=policy_ns,
            ))
            if r.returncode != 0:
                die("failed to apply policy")
            r = kubectl("apply", "-f", "-", input=load_manifest(
                "policy-scaling-pod.yaml",
                index=str(i),
                namespace=policy_ns,
                node=NODE_NAME,
            ))
            if r.returncode != 0:
                die("failed to apply test pod")

            r = kubectl(
                "wait", "pod", "-n", policy_ns,
                "--for=condition=Ready",
                "-l", "security.rancher.io/policy",
                "--timeout=120s",
            )
            if r.returncode != 0:
                log("WARNING: not all policy pods ready within timeout")

        ac, am, mc, mm = sample_agent_metrics()
        controller_ac, controller_am, controller_mc, controller_mm = sample_controller_metrics()

        node_ac, node_am, node_mc, node_mm = sample_node_metrics()

        avg_nopolicy = checkthroughput("no-policy", policy_ns)
        avg_monitor = checkthroughput("monitor", policy_ns, {"security.rancher.io/policy": "benchmark-policy"})
        avg_protect = checkthroughput("protect", policy_ns, {"security.rancher.io/policy": "benchmark-policy"})

        result.append([n, node_ac, node_am, ac, am, controller_ac, controller_am, avg_nopolicy, avg_monitor, avg_protect])

        time.sleep(3)  # let controller sync BPF maps
        r = kubectl("delete", "namespace", policy_ns, "--ignore-not-found")
        if r.returncode != 0:
            die("failed to delete namespace")
    
    report(tabulate(result, headers=policy_header, tablefmt="github"))


    # ------------------------------------------------------------------
    # Section 4: Pod scaling with events
    # ------------------------------------------------------------------

    report("## Pod Scaling with events")
    report()
    report("Pause pods (registry.k8s.io/pause:3.10) are deployed in a dedicated namespace.")
    report()

    pod_scale_ns = "bench-pod-scaling"

    result = []
    for count in pod_testcase:
        log(f"Pod count: {count}")
        recreate_namespace(pod_scale_ns)

        r = kubectl("apply", "-f", "-", input=load_manifest(
            "pause-policy.yaml", namespace=pod_scale_ns,
        ))
        if r.returncode != 0:
            die("failed to apply policy")

        deploy_pause_pods(pod_scale_ns, count)
        if count != 0:
            time.sleep(60)  # wait for metrics-server to pick up new pod state

        ac, am, mc, mm = sample_agent_metrics()
        controller_ac, controller_am, controller_mc, controller_mm = sample_controller_metrics()

        node_ac, node_am, node_mc, node_mm = sample_node_metrics()

        # This will lead to lots of events and this is what we want.
        avg_monitor = checkthroughput("monitor", pod_scale_ns, {"security.rancher.io/policy": "pause-policy"})

        result.append([count, node_ac, node_am, ac, am, controller_ac, node_am, "n/a", avg_monitor, "n/a"])

        r = kubectl("delete", "namespace", pod_scale_ns, "--ignore-not-found")
        if r.returncode != 0:
            die("failed to delete namespace")

    report(tabulate(result, headers=pod_header, tablefmt="github"))

    # ------------------------------------------------------------------
    # Section 5: Metadata
    # ------------------------------------------------------------------
    log("=== Collecting metadata ===")

    # Resolve agent pod's node so all metadata is tied to the sampled node.
    r = kubectl(
        "get", "pod", "-n", AGENT_NS,
        "-l", "app.kubernetes.io/component=agent",
        "--field-selector", f"spec.nodeName={NODE_NAME}",
        "-o", "jsonpath={.items[0].spec.nodeName}",
    )
    agent_node = r.stdout.strip()
    if not agent_node:
        die(f"no agent pod found in {AGENT_NS} namespace")

    agent_image = kubectl(
        "get", "daemonset", "runtime-enforcer-agent", "-n", AGENT_NS,
        "-o", "jsonpath={.spec.template.spec.containers[0].image}",
    ).stdout.strip()

    agent_version = kubectl(
        "get", "daemonset", "runtime-enforcer-agent", "-n", AGENT_NS,
        "-o", "jsonpath={.metadata.labels.app\\.kubernetes\\.io/version}",
    ).stdout.strip() or "unknown"

    chart_version = kubectl(
        "get", "daemonset", "runtime-enforcer-agent", "-n", AGENT_NS,
        "-o", "jsonpath={.metadata.labels.helm\\.sh/chart}",
    ).stdout.strip() or "unknown"

    def node_info(field: str) -> str:
        return kubectl("get", "node", agent_node, "-o", f"jsonpath={{{field}}}").stdout.strip()

    k8s_raw = run(["kubectl", "version"]).stdout
    k8s_version = next(
        (line.split()[-1] for line in k8s_raw.splitlines() if "Server Version" in line),
        "unknown",
    )
    kernel    = node_info(".status.nodeInfo.kernelVersion")
    os_image  = node_info(".status.nodeInfo.osImage")
    runtime   = node_info(".status.nodeInfo.containerRuntimeVersion")
    cpu_cap   = node_info(".status.capacity.cpu")
    mem_cap   = node_info(".status.capacity.memory")

    report(textwrap.dedent(f"""\
        ## Environment

        | Field                   | Value |
        |-------------------------|-------|
        | Enforcer chart          | {chart_version} |
        | Agent version           | {agent_version} |
        | Agent image             | {agent_image} |
        | Kubernetes              | {k8s_version} |
        | Kernel                  | {kernel} |
        | OS image                | {os_image} |
        | Container runtime       | {runtime} |
        | Sampled node            | {agent_node} |
        | Node CPU capacity       | {cpu_cap} |
        | Node memory capacity    | {mem_cap} |
        | Measurement timestamp   | {now} |
        | Benchmark script        | hack/bench/bench.py |

    """))

    # ------------------------------------------------------------------
    # Footer
    # ------------------------------------------------------------------
    report("---")
    report()
    report("*Measurements taken with [hack/bench/bench.py](../../hack/bench/bench.py).*")

    log(f"=== Done. Report: {REPORT_PATH} ===")

    print(f"\nReport written to: {REPORT_PATH}")
    print(REPORT_PATH.read_text())


if __name__ == "__main__":
    try:
        main()
    finally:
        helm("delete", "-n", "runtime-enforcer", "runtime-enforcer", "--ignore-not-found")
        kubectl("delete", "namespace", "runtime-enforcer", "--ignore-not-found", "--wait=false")
        kubectl("delete", "namespace", "bench-baseline", "--ignore-not-found", "--wait=false")
        kubectl("delete", "namespace", "bench-pod-scaling", "--ignore-not-found", "--wait=false")
        kubectl("delete", "namespace", "bench-policy-scaling", "--ignore-not-found", "--wait=false")
