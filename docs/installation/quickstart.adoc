...
* A Kubernetes cluster (for example, link:https://minikube.sigs.k8s.io/docs/[minikube] with CRI-O or containerd; Docker is not supported)
...
helm install otel-collector open-telemetry/opentelemetry-collector \
  ...
  --create-namespace \
  --values ./docs/yaml/otel-config.yaml \
  --wait
```
...
[NOTE]
====
Since we have not yet published a public release and the repo is not yet open source, we need to load the images into the cluster manually.
====
...
# daemon image
make build-daemon-image

# If you are using minikube, load the images into the cluster
minikube image load ghcr.io/neuvector/runtime-enforcer/daemon:latest
minikube image load ghcr.io/neuvector/runtime-enforcer/operator:latest

# Install the Helm chart (using Linux capabilities instead of privileged mode)
helm install runtime-enforcer ./charts/runtime-enforcer \
  --namespace runtime-enforcer \
  --create-namespace \
  --set "daemon.daemon.containerSecurityContext.capabilities.add[0]=SYS_ADMIN" \
  --set "daemon.daemon.containerSecurityContext.capabilities.add[1]=SYS_RESOURCE" \
  --set "daemon.daemon.containerSecurityContext.capabilities.add[2]=SYS_PTRACE" \
...
# Install the Helm chart (using privileged mode)
helm install runtime-enforcer ./charts/runtime-enforcer \
  --namespace runtime-enforcer \
  --create-namespace \
  --set daemon.daemon.containerSecurityContext.privileged=true \
  --set daemon.daemon.containerSecurityContext.allowPrivilegeEscalation=true \
  --set operator.manager.image.tag=latest \
  --set daemon.daemon.image.tag=latest \
  --set telemetry.mode=custom \
  --set telemetry.tracing=true \
...
```bash
# we use the local path since the repo is not published yet
kubectl apply -f ./docs/yaml/ubuntu-deployment.yaml
...
To do that, we label the `WorkloadPolicyProposal` with the appropriate label.
