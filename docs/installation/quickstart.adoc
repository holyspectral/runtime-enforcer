...
If you already have an OTEL collector deployed in your cluster, you can disable the standalone collector and point the agent directly at it:

```bash
helm upgrade runtime-enforcer runtime-enforcer/runtime-enforcer \
  --namespace runtime-enforcer \
  --set agent.violations.collector.enabled=false \
  --set agent.violations.otlpEndpoint=otel-collector.otel-collector.svc.cluster.local:4317 \
  --set telemetry.tracing=true \
