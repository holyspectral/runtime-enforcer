...
- [Runtime Enforcer Resource Consumption](#runtime-enforcer-resource-consumption)
  - [Sizing recommendation](#sizing-recommendation)
    - [Agent](#agent)
    - [Controller](#controller)
  - [Benchmark Results v0.6.0](#benchmark-results-v060)
    - [Environment](#environment)
    - [Baseline](#baseline)
    - [Scaling per pod](#scaling-per-pod)
    - [Policy Scaling](#policy-scaling)
    - [Controller Memory utilization](#controller-memory-utilization)
...
Note: By default, the Runtime Enforcer helm charts deploy these components as `Guaranteed` QoS by specifying the same value in both `requests` and `limits`. Please refer to [Kubernetes Docs](https://kubernetes.io/docs/concepts/scheduling-eviction/node-pressure-eviction/#pod-selection-for-kubelet-eviction) if you want to set different values for `requests` and `limits`. 
...
In addition to these, Runtime Enforcer agent/controller pods need extra resources to handle security events and behavior learning. They also have to handle bursts during policy and workload reconciliation. 

Based on these and the benchmark results, here is our recommendation.  Please consider this as a starting point for your optimization instead of the final configuration, since every cluster could vary.
...
In each test, CPU and memory utilization are collected from [Kubernetes Metrics Server](https://github.com/kubernetes-sigs/metrics-server) for nodes and agent/controller pods. The throughput of execve calls is collected using [Linux perf](https://github.com/torvalds/linux/blob/master/tools/perf/perf.c).

Note: Depending on the timing when the metrics are collected, there could be variance in the numbers, especially in CPU utilization.
